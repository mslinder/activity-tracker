    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        switch (name) {
          ...
          case 'update_workout': {
            if (!args) throw new Error('Arguments required for update_workout');
            const typedArgs = args as { identifier: string; updates: any; replaceExercises?: boolean };
          ...
          case 'delete_workout': {
            if (!args) throw new Error('Arguments required for delete_workout');
            const typedArgs = args as { identifier: string };
