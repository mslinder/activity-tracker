...
class CodeExplorerServer {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            try {
                switch (request.params.name) {
                    ...
                    case 'list_files':
                        return await this.listFiles(request.params.arguments?.directory, request.params.arguments?.pattern, request.params.arguments?.include_content);
                    case 'search_code':
                        return await this.searchCode(request.params.arguments?.query, request.params.arguments?.file_pattern, request.params.arguments?.case_sensitive);
                    case 'get_project_structure':
                        return await this.getProjectStructure(request.params.arguments?.max_depth);
                    case 'analyze_component':
                        return await this.analyzeComponent(request.params.arguments?.component_path);
                    default:
    ...
    async listFiles(directory = '.', pattern, includeContent = false) {
    ...
    async searchCode(query, filePattern = '**/*.{ts,tsx,js,jsx}', caseSensitive = false) {
    ...
    async buildDirectoryTree(dirPath, prefix, currentDepth, maxDepth) {
        if (currentDepth >= maxDepth)
            return '';
        let result = '';
        try {
            const items = await fs.readdir(dirPath);
            const filteredItems = this.gitignore.filter(items.map(item => path.relative(PROJECT_ROOT, path.join(dirPath, item)))).map(item => path.basename(item));
